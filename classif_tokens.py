...
def load_file(path):
    f_train = glob(path+'/train/*.txt')
    f_dev = glob(path+'/dev/*.txt')
    my_dict = {"train":[], "dev":[]}
    for i,f in enumerate(f_train):
        my_dict["train"] += {'id':i, 'tags':[], 'tokens':[]}
    for i,f in enumerate(f_dev):
        my_dict["dev"] += {'id':i, 'tags':[], 'tokens':[]}
